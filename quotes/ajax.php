...
} elseif ($ACTION == 'search' && !empty($TYPE)) {
	try {
		$db = new MySQL();
		if ($TYPE == 'global') {
			$db->sfquery(array('SELECT * FROM `%s` WHERE quote REGEXP "'.$QUERY.'"',MYSQL_TABLE));
		} elseif ($TYPE == 'user') {
			if (!empty($ID)) {
				$db->sfquery(array('SELECT * FROM `%s` WHERE quote REGEXP "'.$QUERY.'" AND owner_id = %s',MYSQL_TABLE,$ID));
			} else {
				$db->sfquery(array('SELECT * FROM `%s` WHERE quote REGEXP "'.$QUERY.'" AND owner_id = %s',MYSQL_TABLE,$UID));
			}
		}
		if ($db->numRows()) {
